...
10. **Docs are part of done.** Like the TDD mandate — docs ship with the change, not after. User-facing endpoint or config → generated reference covers it (API ref from OpenAPI; env/config ref from `config.rs`) + ≥ skeletal narrative docs in the feature PR. CI gates the docs build. Mechanism: UNK-370.
