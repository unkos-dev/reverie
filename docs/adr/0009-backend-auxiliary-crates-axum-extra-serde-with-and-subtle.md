...
RFC 7396 JSON Merge Patch body so a `PATCH` handler can tell a field left out of the body apart from one set to `null`,
and a constant-time comparison for a presented secret against a stored one so a CSRF token check does not leak timing
information. Which crates should serve those three needs?
...
- Preferring an existing dependency's feature flag over a new crate, and an audited crate over a hand-rolled primitive,
  keeps the dependency surface and the maintenance burden small.
...
- Positive: the `axum-extra` feature addition is the minimum diff that supports repeated-key filter parameters, with no
  new dependency and no version churn.
...
- Negative: `axum-extra` is already in tree; enabling its `"query"` feature is the smaller diff, and adding `serde_qs`
  as a sibling top-level dependency would cover substantially the same surface at the cost of a new crate.

### Hand-rolled `Option<Option<T>>` Merge Patch decode

- Negative: `serde_with::rust::double_option` already is this, written, tested, and audited, and reused across the serde
  ecosystem; hand-rolling a `Visitor` per field is not a decode primitive Reverie should own.

### Replace `subtle` with a hand-rolled constant-time compare

- Negative: `subtle` is already in tree, and reinventing its optimisation-barrier protection opens the comparison to the
  same class of compiler-rewrite bug the crate explicitly defends against.

### A third-party query-string extractor crate

- Negative: rejected on the same "already in tree" grounds as `serde_qs`; the `axum-extra` `"query"` feature is the path
  of least resistance.
...
Sibling ADR: [JSON API conventions](./0011-json-api-conventions-for-the-browser-facing-rest-surface.md) (the RFC 7396
Merge Patch decision and the CSRF synchroniser-token decision).
