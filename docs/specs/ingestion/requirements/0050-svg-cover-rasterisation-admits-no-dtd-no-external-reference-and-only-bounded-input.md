...
profile-version: 1
id: "REV-REQ-0050"
title: "SVG cover rasterisation admits no DTD, no external reference and only bounded input"
governed-by:
  - "REV-ADR-0024"
---

# SVG cover rasterisation admits no DTD, no external reference and only bounded input

## Statement

WHEN the cover service rasterises an SVG cover, it MUST refuse a document that carries a DOCTYPE, MUST resolve an
...
bound; a rasterisation whose output carries no visible pixel MUST also be refused.
...
- A rasterisation whose rendered output carries no visible (non-zero-alpha) pixel is refused. Checked by
...
- Each bound below is enforced at exactly the stated limit:
  ...
  | Cumulative decoded pixels admitted per rasterisation | 67,108,864 pixels (64 megapixels) | `caps_cumulative_decoded_pixels`; `decoded_pixel_budget_is_shared_across_resolvers` |
  | Sibling-image resolutions per rasterisation | 8 resolutions | `caps_sibling_resolution_count` (resolutions past the cap are refused; the document still renders with the admitted images rather than being rejected outright) |
