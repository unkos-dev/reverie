...
its intent to that writer, and a component on the library route MUST NOT read or write a library filter parameter by any
other path.

## Rationale

A shared URL parameter set with more than one independent writer lets two surfaces build on stale copies of the same key
and clobber each other's edits, which is the hazard the project's shared-mutable-state rule exists to close. Naming a
single writer removes the possibility of a read split or a lost write by construction and confines cross-surface
coordination to that one component. The mechanism that makes a second reader unsafe here is described in the Design that
