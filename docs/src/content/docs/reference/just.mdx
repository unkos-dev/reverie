...
| `just worktree <branch>`       | Create a git worktree for BRANCH at `$WORKTREE_ROOT/reverie/<slug>`, with an isolated cargo target dir and, when present, `.claude/settings.local.json`, `.codex/config.toml`, and `.codex/rules/` overlays carried over; warns if CARGO_TARGET_DIR or CARGO_BUILD_TARGET_DIR would override it. |
...
| `just infra::worktree-cargo-target-selftest`     | Self-test the worktree recipe's isolated cargo target-dir config.                                                 |
