...
| `just worktree <branch>`       | Create a git worktree for BRANCH at `$WORKTREE_ROOT/reverie/<slug>`, with an isolated cargo target dir; warns if CARGO_TARGET_DIR or CARGO_BUILD_TARGET_DIR would override it. |
