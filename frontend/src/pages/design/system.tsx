  ...
  return (
    <section className="border-b border-border last:border-b-0 py-8">
      <header className="mb-4">
        ...
        {description ? <p className="text-fg-muted text-sm mt-1">{description}</p> : null}
      </header>
 ...
 *
 * Dev-only gallery that renders every shadcn primitive used by Reverie
 * (typography, buttons, form controls, surfaces, badges, skeletons) plus
 * the canonical token palette and the brand's no-hue state expressions
 * (error / disabled / hover / selected) so the brand-aligned aliasing in
 * `styles/themes/index.css` can be sighted against the actual primitives
    ...
    <main className="bg-canvas text-fg min-h-screen">
      <header className="bg-canvas-2 border-b border-border px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          <Lockup size={28} theme={effective === "dark" ? "dark" : "light"} />
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <Example label="Body">
            <p className="font-body text-base text-fg">
              The Reverie design system codifies brand identity into utility tokens. Every surface
              inherits the canonical palette.
            </p>
          </Example>
        ...
        <Section
          title="Buttons"
          description="Brand rule: the bg-accent gold fill (A77C00 on Light) is reserved for large CTAs and recovery actions as a brand restriction, not a contrast concession. The fill carries contrast through its Ink text (5.11:1 on Light, AA at any size); gold that must read as text or a hairline uses the higher-contrast accent-text step. Default-size actions use the outline variant; lg size unlocks the gold fill."
        >
          ...
          <Example label="Loading (opacity-pulse)">
            <Button size="lg" className="animate-[loading-pulse_1.6s_ease-in-out_infinite]">
              Saving…
            </Button>
          </Example>
        </Section>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-fg-muted text-sm">
                  A novel about guilt, morality, and the boundaries of conscience.
                </p>
              </CardContent>
              <CardFooter>
                <Button size="sm" variant="outline">
        ...
        >
          {[
            ...
            <Example key={token} label={`--color-${token}`}>
              <div className={`bg-${token} h-12 w-full border border-border rounded-sm`} />
            </Example>
          ))}
        </Section>
        ...
        >
          <Example label="Error pattern (font-semibold + gold recovery)">
            <div className="flex flex-col gap-2">
              <p className="text-fg font-semibold">Could not save the shelf.</p>
          ...
          <Example label="Disabled (opacity-50 + text-fg-muted, aria-disabled)">
            <p className="opacity-50 text-fg-muted" aria-disabled="true">
              Unavailable in offline mode
            </p>
          </Example>
          <Example label="Hover (bg-hover, surface-2 lift)">
            <span className="bg-hover text-fg rounded-sm px-2 py-1 text-sm">Hovered row</span>
          </Example>
