/*
 * Vendored from radix-ui/website @ 88a9f14 (components/generate-radix-colors.tsx).
 * MIT License, Copyright (c) 2024 WorkOS. Logic is unmodified; the file is
 * renamed to .ts (no JSX) and reformatted by the repository formatter.
 *
 * This is the color engine behind src/styles/themes/primitives.generated.css.
 * The Reverie-specific anchors and overrides live in emit-primitives.ts.
 *
 * ts-nocheck: upstream is written against looser colorjs.io typings (coords
 * as number, not number | null). The drift test pins the emitted artifact, so
 * behavioral regressions surface there rather than through typechecking.
 */
// @ts-nocheck
