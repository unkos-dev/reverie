...
- The brand carries a deliberate constraint: on Parchment the light-theme gold measures 2.80:1, clearing neither the 1.4.11 non-text boundary nor 1.4.3 at any text size. It is never used as a hairline or as text. As a fill it pairs with Ink at 5.11:1, which clears 1.4.3 for normal text, and the focus ring uses a separate darker accent-text shade that clears the non-text boundary on its own. The accent fill stays restricted on light surfaces to large CTAs and recovery actions as a brand rule rather than an accessibility concession. axe-core contrast violations on a gold edge or gold text are the right signal: the surface is misusing the accent.
