...
**The Cover-Spine Rule.** Real cover art is canonical; when a book has none (or the art fails to load), the grid and detail surfaces render a typographic spine (`CoverArtwork`): five compositions × four colorways assigned deterministically from the book id. Covers are brand-fixed and theme-independent — the `--cover-*` tokens are declared once on `:root` with no `[data-theme]` override, because a publisher's spine does not shift with the reader's theme. Every `text-cover-*` foreground is paired with its `bg-cover-*` ground inside the spine's own composition (`COLORWAY_CLASSES`), and every pairing clears 4.5:1 by construction. Because `--cover-parchment` equals the Light canvas, covers carry a pedestal (hairline + shadow) on Light so books stay objects on Parchment. Tooling note: axe cannot attribute the spine's ground through its absolutely-positioned layers and falls back to page-canvas contrast — such reports on `text-cover-*` nodes are background-attribution false positives, carved out in `scripts/a11y/allowlist.mjs`.
