...
- **Library URL State:** The library's filter and view state lives in typed per-key search params (`lib/hooks/use-library-filters.ts`). Every read and every write on that surface goes through it. React Router's `useSearchParams` is banned there by lint, in both directions: those writes reach the URL through `history.replaceState`, which the router's history does not observe, so a read through it returns filter state that goes stale as soon as anything writes, and a write through it lands where the surface cannot read it.
- **Library Sort Is Not URL State:** Sort is a one-layer per-user preference (`docs/adr/0047-library-sort-is-a-per-user-preference-never-url-state.md`): every sort gesture writes `sort_stack` through the page's one intent handler, every surface reads the one resolved stack, and the list request carries the override explicitly. Never add a `?sort=` reader or writer to the library surface, never send the installation default sort in a request, and never resolve sort independently in a component: a second resolution is the read split that made an inherited descending sort unclearable.
