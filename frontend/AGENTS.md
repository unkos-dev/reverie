...
3. **shadcn via CLI only:** Do not manually create or paste shadcn/ui components. Run `npx --no-install shadcn add <component>` so the CLI resolves from the lockfile-backed local dependency.
...
- **Strict Effects:** `useEffect` must have a complete dependency array. Never suppress the linter or pass an async function directly. Server-state fetching belongs in React Query, not an Effect. Rare async Effects for non-server-state synchronization must cancel obsolete work, using `AbortController` when the API supports it, and ignore cancellation during error handling.
