...
- [Lint type-semantics and the hoisted typescript trail the 7.0 compiler](2026-07-19-typescript-7-vite-plus-pairing-lag.md): adopted with the typescript ~7.0.0 bump; vite-plus exact-pins oxlint-tsgolint =0.24.0 (pre-GA typescript-go snapshot) and its ^5||^6 typescript peer keeps a hoisted 6.0.3 beside the frontend's 7.0.2; lifts when a vite-plus release ships tsgolint >=0.25.0 with a ^7 peer and the tree resolves a single typescript
