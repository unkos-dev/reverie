...
- [Folded search indexes trust a dictionary Postgres may change](2026-08-02-unaccent-dictionary-reindex-obligation.md): adopted 2026-08-02; accent-insensitive matching needs unaccent() inside expression indexes and Postgres refuses STABLE functions there, so the immutable_unaccent wrapper hides dictionary drift and every Postgres major bump against a persistent database must REINDEX the folded indexes and recompute works.search_vector; lifts when upstream ships a versioned or honestly immutable folding dictionary
