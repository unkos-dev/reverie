...
pipeline, per-cell and per-row locks, and virtualised scrolling over 50K-plus
...
- API stability and maintenance cadence outweigh bundle size, because the grid's
  surface is a long-lived integration contract while bytes are amortisable
...
- The evaluation rig must ship zero surface to production.
...
- **TanStack Table + a virtualiser** (DIY): lightest and license-clean, but the
...
- Mount: grid interactive under 1 s from route render, with data pre-generated so
  the number isolates grid initialisation.
...
1. Both candidates clear the hard budgets: the tiebreak favours API stability and
...
under More Information; the ADR flipped to `accepted` once the harness run
completed.
...
- Good, because the rig lives on an unmerged spike branch, adding zero surface
  to main; its react-data-grid binding, theme bridge, and perf instrumentation
  are cherry-pickable when the production grid phase starts.
- Bad, because the spike branch carries both candidate libraries as dev
  dependencies, doubling the install and review surface for the life of the
  bake-off.
...
grid is imported outside a binding that satisfies it. The bake-off harness and
both bindings stay on the unmerged spike branch; neither grid library is a
dependency of main until the production grid phase adopts the winner.
...
hard budget, so the other wins).** Measured on the dev harness at 50K rows with
simulated API latency of 0 ms, driven via agent-browser.
...
would have pointed at AG Grid. The verdict hangs on that scroll call, resolved
by applying the guardrail as written and logging the stale selector as a rig
defect rather than re-running.
