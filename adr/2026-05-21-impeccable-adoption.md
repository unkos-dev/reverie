...
- `frontend/package.json` — add `impeccable` devDep, `detect`
  script, `puppeteer.skipDownload` config field
- `frontend/package-lock.json` — generated by `npm install`
- `lint-staged.config.js` — add frontend-gated full-scan entry
  (function form, advisory `|| true`)
- `.github/workflows/ci.yml` — add `Detect (impeccable, advisory)`
  step to the frontend job between Stylelint and font-integrity,
  `continue-on-error: true`
- `adr/README.md` — append this ADR to the index

### Patterns to follow

- `frontend/package.json` keeps single-quoted CLI strings in
  scripts (e.g. `stylelint 'src/**/*.css'`) — match the style
- `lint-staged.config.js` uses string commands; function form is
  fine when needed (justified inline with a brief comment)
- `.github/workflows/ci.yml` frontend job uses `defaults.run.working-directory: frontend`
  — new step inherits that, runs `npm run detect`
- Conventional Commits: `feat(ci): wire impeccable detect into pre-commit + CI (advisory)`

### Tests

- Unit-test surface: none. `impeccable` is a tool, not application
  code. Verification is integration-level:
  - `npm run detect` runs locally with exit 0 on clean tree, exit 2
    with findings
  - Pre-commit `git commit` triggers lint-staged on a staged
    `frontend/src/**.tsx` change, runs detect, doesn't block
    (advisory `|| true`)
  - CI step appears in the frontend job, runs, reports findings,
    doesn't fail the job
  - `npm ci` does NOT download Chromium (verify via timing +
    absence of `chromium-*` under `node_modules/@puppeteer/browsers/cache`
    or equivalent; install completes faster than baseline)

## Verification

- [ ] `frontend/package.json` lists `impeccable` in
      `devDependencies`, `detect` in `scripts`, and
      `"puppeteer": { "skipDownload": true }` at the top level
- [ ] `lint-staged.config.js` triggers
      `npm --prefix frontend run detect || true` on staged
      `frontend/src/**/*.{ts,tsx,html,css}`
- [ ] `.github/workflows/ci.yml` has `Detect (impeccable, advisory)`
      step in the `frontend` job with `continue-on-error: true`
- [ ] `npm run detect` exits 2 against current `main` (3 known
      `bg-black` findings) and exits 0 after the deferred fix lands
- [ ] `npm ci` install does not fetch Chromium — verifiable by
      inspecting install logs for `@puppeteer/browsers` cache
      activity, or by absence of Chromium binary under puppeteer's
      install path
- [ ] `adr/README.md` index updated with this ADR
- [ ] Memory `project_bg_black_overlays_deferred` documents the
      lift condition (first PR touching modal/dialog/sheet); fix
      lands together with stripping both advisory flags
