...
The installer is for the first install only. The global `vp` runs its own bundled oxlint and vitest rather than the project's copies, so after a `vite-plus` bump it has to move to the new catalog pin, or `just js::oxlint` fails on lint rules its older oxlint does not know. Move it with `vp upgrade <version>`. `just doctor` reports the drift and prints that command with the pinned version filled in.
