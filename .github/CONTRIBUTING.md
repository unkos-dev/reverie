...
npm install scripts are gated by the `allowScripts` field in the root `package.json`, and `strict-allow-scripts` in the root `.npmrc` makes an unreviewed one fail the install rather than warn. `npm install-scripts ls` lists anything awaiting a decision, and `npm install-scripts prune` drops entries that no longer match a package in the tree.

Which decision to record depends on who controls the version. For a dependency you pin yourself, review what the script does and run `npm install-scripts approve <pkg>`: that writes a `pkg@version` entry, so a later bump fails the install until someone re-reviews it, which is the behaviour you want. For a dependency whose version you do not control, typically a transitive one resolved through somebody else's range, use `npm install-scripts deny <pkg>` instead. A deny is always written name-only, so it keeps applying as the resolved version moves; a pinned approval on such a package silently stops matching the first time the range resolves somewhere new. Deny also wins over allow wherever both match.

Commit the resulting `package.json` change in the same PR. The image build runs `npm ci --ignore-scripts`, which bypasses the policy entirely, so it is unaffected either way.
