...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 404, description = "OIDC is not configured on this instance", body = crate::openapi::ProblemDetails, content_type = "application/problem+json"),
...
#[utoipa::path(
    ...
    description = "Completes the OIDC authorization-code flow, establishes a session, and redirects to the app. No authentication is required to call it. Returns 401 if the anti-forgery state does not match or the ID token fails validation.",
    tag = "auth",
    security(()),
...
#[utoipa::path(
    ...
    description = "Creates the first administrator account during initial setup and does not sign the caller in. No authentication is required to call it. Returns 409 if an administrator already exists and 429 when too many requests have been made.",
    tag = "auth",
    security(()),
...
#[utoipa::path(
    ...
    description = "Creates a new account with the `adult` role and does not sign the caller in. No authentication is required to call it. Returns 404 if self-registration or local authentication is disabled, 409 if the email is already in use, and 429 when too many requests have been made.",
    tag = "auth",
    security(()),
...
#[utoipa::path(
    ...
    responses(
        ...
        (status = 403, description = "X-CSRF-Token header present but does not match the session token", body = crate::openapi::ProblemDetails, content_type = "application/problem+json"),
        (status = 428, description = "X-CSRF-Token header required for a session-authenticated caller", body = crate::openapi::ProblemDetails, content_type = "application/problem+json"),
        (status = "default", description = "Failures raised by a handler or its extractors are Problem Details documents", body = crate::openapi::ProblemDetails, content_type = "application/problem+json")
    )
)]
...
#[utoipa::path(
    ...
    description = "Returns the authenticated caller's profile, including the `csrf_token` to echo back as `X-CSRF-Token` on unsafe requests (`null` for sessions authenticated with OPDS Basic auth).",
    tag = "auth",
    responses(
...
#[utoipa::path(
    ...
    description = "Persists the caller's theme preference and refreshes the `reverie_theme` cookie.",
    tag = "auth",
