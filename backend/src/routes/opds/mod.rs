//! OPDS 1.2 catalog routes.
//!
//! Mount under `/opds/*` with a Basic-only extractor (RFC 7617), plus a
//! dual-mount cover handler at `/api/v1/books/:id/cover{,/thumb}` behind
//! cookie-or-Basic for the web UI.
//!
...
/// OPDS-mount cover handlers), unconditionally.
///
/// The OpenAPI spec is built
...
/// The runtime half of [`openapi_router`].
///
...
/// The `/api/v1/books/:id/cover{,/thumb}` mount.
///
/// Behind the cookie-or-Basic
