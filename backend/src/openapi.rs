...
#[openapi(
    ...
    components(schemas(
        ProblemDetails,
        // Reached only through a `schema_with` field (see the wrapper
        // functions below): `routes!` cannot auto-collect a schema that
        // never appears as a plain `ToSchema` field.
        crate::models::contributor_role::ContributorRole,
        crate::models::library::SearchHitKind,
        crate::models::external_identifier::IdentifierLevel,
        crate::routes::metadata::MetadataContributors,
        crate::models::enrichment_status::EnrichmentStatus,
        crate::models::ingestion_status::IngestionStatus,
        crate::models::validation_status::ValidationStatus,
        crate::models::library::MetadataVersionSummary
    )),
    tags(
...
/// Schema for a `serde_json::Value` field carrying an untyped
/// source-reported value: the actual shape varies per metadata field
/// (string, number, object, ...), so the only accurate `type` is the full
/// JSON-Schema union rather than a single guessed type. `description` is
/// carried on the returned object because these fields are `schema_with`
/// sites, which bypass the field's own doc comment.
fn any_json_value_schema(description: &str) -> utoipa::openapi::schema::Object {
    use utoipa::openapi::schema::{ObjectBuilder, SchemaType, Type};

    // `Integer` is deliberately absent: JSON Schema treats every integer as
    // a `Number`, and including `Integer` here would make `owasp-integer-
    // format` demand an `int32`/`int64` format that is meaningless across a
    // union that also allows strings and objects.
...
/// `#[schema(schema_with = ...)]` site for `FieldChange::new_value`.
pub(crate) fn field_change_new_value_schema() -> utoipa::openapi::schema::Object {
    any_json_value_schema("The proposed new value in its raw JSON form.")
}

/// `#[schema(schema_with = ...)]` site for `FieldVersionChange::value`.
pub(crate) fn field_version_change_value_schema() -> utoipa::openapi::schema::Object {
    any_json_value_schema(
        "Canonical value as applied, after server normalization (null when the \
         field was cleared).",
    )
}

/// `#[schema(schema_with = ...)]` site for `MetadataRow::new_value`.
pub(crate) fn metadata_row_new_value_schema() -> utoipa::openapi::schema::Object {
    any_json_value_schema("Proposed value in raw JSON form.")
}

/// `#[schema(schema_with = ...)]` site for `MetadataVersionRow::new_value`.
pub(crate) fn metadata_version_row_new_value_schema() -> utoipa::openapi::schema::Object {
    any_json_value_schema(
...
/// A bare `$ref` with no sibling keywords. utoipa's field-level `#[schema]`
/// attribute has no way to suppress the description a doc comment
/// contributes, so a field that must keep its doc comment for
/// `#![deny(missing_docs)]`, but whose doc text is redundant with the
/// referenced schema's own description, routes through `schema_with` to a
/// function returning this instead.
fn bare_ref(schema_name: &str) -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    utoipa::openapi::Ref::from_schema_name(schema_name).into()
}

/// `#[schema(schema_with = ...)]` site for the `enrichment_status` field on
/// `WorkManifestation`, `BookListRow`, and `BookDetail`.
pub(crate) fn enrichment_status_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("EnrichmentStatus")
}

/// `#[schema(schema_with = ...)]` site for the `ingestion_status` field on
/// `WorkManifestation`, `BookListRow`, and `BookDetail`.
pub(crate) fn ingestion_status_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("IngestionStatus")
}

/// `#[schema(schema_with = ...)]` site for the `validation_status` field on
/// `WorkManifestation`, `BookListRow`, and `BookDetail`.
pub(crate) fn validation_status_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("ValidationStatus")
}

/// `#[schema(schema_with = ...)]` site for `BookDetail::metadata_version_summary`.
pub(crate) fn metadata_version_summary_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("MetadataVersionSummary")
}

/// `#[schema(schema_with = ...)]` site for `Settings::cleanup_mode`.
pub(crate) fn cleanup_mode_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("CleanupMode")
}

/// `#[schema(schema_with = ...)]` site for `ContributorRef::role`. The field
/// doc comment stays for rustdoc; a single-item `allOf` was tried to carry
/// it into the schema too, but vacuum's `no-unnecessary-combinator` (already
/// adopted) rejects a combinator with only one item, so the two rules leave
/// no way to keep both a component-specific description and a `type` here.
pub(crate) fn contributor_ref_role_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("ContributorRole")
}

/// `#[schema(schema_with = ...)]` site for `ExternalIdRef::level`.
pub(crate) fn external_id_ref_level_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("IdentifierLevel")
}

/// `#[schema(schema_with = ...)]` site for `SearchHit::kind`.
pub(crate) fn search_hit_kind_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("SearchHitKind")
}

/// `#[schema(schema_with = ...)]` site for `PreferenceDefaults::view`.
pub(crate) fn preference_defaults_view_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("LibraryView")
}

/// `#[schema(schema_with = ...)]` site for `PreferenceDefaults::density`.
pub(crate) fn preference_defaults_density_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema>
{
    bare_ref("LibraryDensity")
}

/// `#[schema(schema_with = ...)]` site for `BookMetadata::contributors`.
pub(crate) fn book_metadata_contributors_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema>
{
    bare_ref("MetadataContributors")
}

/// `#[schema(schema_with = ...)]` site for `UpdateThemeRequest::theme_preference`.
pub(crate) fn update_theme_request_theme_preference_schema()
-> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("ThemePreference")
}

/// `#[schema(schema_with = ...)]` site for `UpdateRoleRequest::role`.
pub(crate) fn update_role_request_role_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("Role")
}

/// `#[schema(schema_with = ...)]` site for `CreateUserRequest::role`.
pub(crate) fn create_user_request_role_schema() -> utoipa::openapi::RefOr<utoipa::openapi::Schema> {
    bare_ref("Role")
}
