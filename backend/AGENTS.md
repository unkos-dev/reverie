...
- **Time Crate:** Use the `time` crate. The `chrono` crate is strictly forbidden in first-party code. The reason is consistency, not risk: one date/time crate avoids conversion layers and ambiguity about which type an API wants. Both crates are supported by `sqlx` and `utoipa`; `time` is the one this repository wires up, and `backend/Cargo.toml` leaves the `chrono` features off deliberately. `chrono` is well maintained, and it is already in the production dependency graph transitively via `openidconnect` and `jwks_client_rs`, so this rule is about first-party code, not about keeping the crate out of the build. The carve-out is `test_support.rs::oidc_mock`, where `openidconnect` v4's `CoreIdTokenClaims::new` forces `chrono` types on the call site; `chrono` use must not leave that function. Note what the guard actually covers: `scripts/backend-guards.sh` greps `backend/src/` for `use chrono` and `extern crate chrono` against `backend/guards/chrono-allowlist.txt`, whose entry is a bare file path. So the mechanical check exempts all of `test_support.rs`, and a fully-qualified `chrono::` path is caught nowhere. The rule above is binding regardless of what the grep sees.
