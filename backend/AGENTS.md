...
- **Time Crate:** Use the `time` crate. The `chrono` crate is strictly forbidden in first-party code. The reason is consistency, not risk: one date/time crate avoids conversion layers and ambiguity about which type an API wants, and `time` is the one with first-party `sqlx` and `utoipa` support. `chrono` is well maintained, and it is already in the production dependency graph transitively via `openidconnect` and `jwks_client_rs`, so this rule is about first-party code, not about keeping the crate out of the build. The carve-out is `test_support.rs::oidc_mock`, where `openidconnect` v4's `CoreIdTokenClaims::new` forces `chrono` types on the call site. `backend/guards/chrono-allowlist.txt` registers the exemption and `scripts/backend-guards.sh` enforces it at file granularity, so confining the use to `oidc_mock` is a convention the guard does not check.
