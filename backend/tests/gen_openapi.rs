...
/// `operation-operationId-unique` (vacuum): utoipa-axum derives an
/// `operationId` from the handler function name by default, so two
/// unrelated handlers named `detail` collided.
#[test]
fn operation_ids_are_unique() {
    ...
    for (path, methods) in doc["paths"].as_object().expect("paths object") {
        for (method, op) in methods.as_object().expect("path item object") {
            let Some(id) = op.get("operationId").and_then(|v| v.as_str()) else {
                continue;
            };
            if let Some(previous) = seen.insert(id.to_string(), format!("{method} {path}")) {
                panic!("operationId {id:?} used by both {previous} and {method} {path}");
            }
        }
    }
}

/// `owasp-integer-format` (vacuum): an integer property must declare
/// `int32`/`int64` so clients size it correctly. `usize` is pointer-width,
/// which is 64-bit on every platform this API ships on.
#[test]
fn scan_response_counters_declare_int64_format() {
    let rendered = reverie_api::openapi::spec_json().expect("serialize OpenAPI spec");
    let doc: serde_json::Value = serde_json::from_str(&rendered).expect("valid JSON");

    let props = &doc["components"]["schemas"]["ScanResponse"]["properties"];
    for field in ["processed", "failed", "skipped"] {
        assert_eq!(
            props[field]["format"],
            serde_json::json!("int64"),
            "ScanResponse.{field} must declare an int64 format"
        );
    }
}

/// `oas-missing-type` (vacuum): every schema property must carry its own
/// `type`, or be a bare `$ref` (nothing else in the object — the rule does
/// not resolve refs), or use a composite keyword (`oneOf`/`allOf`/`anyOf`,
/// which does not require its own `type`). A `$ref` combined with a
/// sibling `description` satisfies none of those.
#[test]
fn schema_properties_declare_a_type() {
    let rendered = reverie_api::openapi::spec_json().expect("serialize OpenAPI spec");
    let doc: serde_json::Value = serde_json::from_str(&rendered).expect("valid JSON");
    ...
    for (schema_name, schema) in schemas {
        ...
        for (prop_name, prop) in properties {
            ...
            let is_bare_ref = obj.contains_key("$ref") && obj.len() == 1;
            let is_composite =
                obj.contains_key("oneOf") || obj.contains_key("allOf") || obj.contains_key("anyOf");
            if !(has_type || is_bare_ref || is_composite) {
                offenders.push(format!("{schema_name}.{prop_name}"));
            }
        }
    }
    assert!(
        offenders.is_empty(),
        "schema properties missing an explicit type: {offenders:?}"
    );
}
