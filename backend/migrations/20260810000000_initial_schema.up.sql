...
CONSTRAINT authors_created_at_ts_decode_range CHECK (((created_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
...
'(transaction-local) before queries — see db::acquire_with_rls. The writeback worker connects via a dedicated pool '
'that sets app.system_context = ''writeback'' (session-scoped via after_connect). reverie_ingestion has unconditional '
'access. reverie (owner) bypasses RLS.';

COMMENT ON COLUMN public.manifestations.ingestion_file_hash IS
'SHA-256 of file at ingestion time. App-layer invariant: never updated after initial insert — audit trail. No schema '
'constraint enforces this; see services::writeback::orchestrator.';

COMMENT ON COLUMN public.manifestations.current_file_hash IS
'SHA-256 of file as of last successful writeback. Equals ingestion_file_hash until first writeback. Step 11 health '
'surfaces divergence from on-disk hash.';
...
CONSTRAINT reading_state_progress_pct_range CHECK (((progress_pct IS NULL) OR ((progress_pct >= (0)::double precision)
AND (progress_pct <= (100)::double precision)))),
...
'RLS enabled. reverie_app and reverie_readonly must call set_config(''app.current_user_id'', ..., true) '
'(transaction-local) before queries — see db::acquire_with_rls. Each user sees only rows where user_id matches the '
'GUC. reverie (owner) bypasses RLS.';
...
parent_id uuid,
created_at timestamp with time zone DEFAULT now() NOT NULL,
CONSTRAINT series_created_at_ts_decode_range CHECK (((created_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (created_at < '10000-01-01 00:00:00+00'::timestamp with time zone)))
);
...
added_at timestamp with time zone DEFAULT now() NOT NULL,
"position" integer DEFAULT 0 NOT NULL,
CONSTRAINT shelf_items_added_at_ts_decode_range CHECK (((added_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (added_at < '10000-01-01 00:00:00+00'::timestamp with time zone)))
);
...
created_at timestamp with time zone DEFAULT now() NOT NULL,
updated_at timestamp with time zone DEFAULT now() NOT NULL,
CONSTRAINT shelves_created_at_ts_decode_range CHECK (((created_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (created_at < '10000-01-01 00:00:00+00'::timestamp with time zone))),
CONSTRAINT shelves_updated_at_ts_decode_range CHECK (((updated_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (updated_at < '10000-01-01 00:00:00+00'::timestamp with time zone)))
);
...
sort_stack text,
updated_at timestamp with time zone DEFAULT now() NOT NULL,
CONSTRAINT user_preferences_hidden_columns_bounded CHECK ((array_length(hidden_columns,
1) <= 64)),
...
CONSTRAINT chk_child_role_sync CHECK ((((is_child = true) AND (role = 'child'::public.user_role)) OR ((is_child = false)
AND (role <> 'child'::public.user_role)))),
CONSTRAINT users_created_at_ts_decode_range CHECK (((created_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (created_at < '10000-01-01 00:00:00+00'::timestamp with time zone))),
CONSTRAINT users_disabled_at_ts_decode_range CHECK (((disabled_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
...
CONSTRAINT users_updated_at_ts_decode_range CHECK (((updated_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (updated_at < '10000-01-01 00:00:00+00'::timestamp with time zone)))
);
...
'RLS enabled (default deny). No handlers implemented yet. Access scoped through webhooks ownership. Define policies '
...
'Short-TTL dedupe set for terminal webhook events (UNK-98). Keyed by stable event id (writeback:{job_id}:{outcome}). '
'Rows past the TTL are purged opportunistically on dispatch; see services::writeback::events::dispatch.';
...
CONSTRAINT works_created_at_ts_decode_range CHECK (((created_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (created_at < '10000-01-01 00:00:00+00'::timestamp with time zone))),
CONSTRAINT works_updated_at_ts_decode_range CHECK (((updated_at >= '0001-01-01 00:00:00+00'::timestamp with time zone)
AND (updated_at < '10000-01-01 00:00:00+00'::timestamp with time zone)))
);
...
CONSTRAINT writeback_jobs_reason_chk CHECK ((reason = ANY (ARRAY['metadata'::text,
'cover'::text])))
);
...
ADD CONSTRAINT field_locks_pkey PRIMARY KEY (manifestation_id,
entity_type,
field_name);
...
ADD CONSTRAINT manifestation_external_identifiers_pkey PRIMARY KEY (manifestation_id,
scheme);

ALTER TABLE ONLY public.manifestation_external_ratings
ADD CONSTRAINT manifestation_external_ratings_pkey PRIMARY KEY (manifestation_id,
source);

ALTER TABLE ONLY public.manifestation_genres
ADD CONSTRAINT manifestation_genres_pkey PRIMARY KEY (manifestation_id,
genre_id);
...
ADD CONSTRAINT metadata_versions_mfs_hash_unique UNIQUE (manifestation_id,
source,
field_name,
value_hash);
...
ADD CONSTRAINT omnibus_contents_pkey PRIMARY KEY (omnibus_manifestation_id,
contained_work_id);
...
CREATE INDEX idx_authors_sort_name_id ON public.authors USING btree (sort_name,
id);
...
CREATE INDEX idx_manifestations_pages_keyset ON public.manifestations USING btree (pages,
id);

CREATE INDEX idx_manifestations_pages_keyset_desc ON public.manifestations USING btree (pages DESC NULLS LAST,
id DESC);
...
CREATE INDEX idx_manifestations_recent_keyset ON public.manifestations USING btree (created_at DESC,
id DESC);
...
CREATE INDEX idx_mv_manifestation_field ON public.metadata_versions USING btree (manifestation_id,
field_name);
...
'Enforces at most one active (unconsumed) recovery PIN per user. Load-bearing for the single-active invariant under '
'concurrency; see models::password_reset_pin::rotate for the retry path on SQLSTATE 23505.';
...
CREATE INDEX idx_series_sort_name_id ON public.series USING btree (sort_name,
id);

CREATE INDEX idx_series_works_series_position ON public.series_works USING btree (series_id,
"position");
...
CREATE INDEX idx_shelves_user_keyset ON public.shelves USING btree (user_id,
is_system DESC,
name,
id);
...
CREATE INDEX idx_work_authors_work_position ON public.work_authors USING btree (work_id,
"position");
...
CREATE INDEX idx_works_first_author_sort_id ON public.works USING btree (first_author_sort_name,
id);
...
CREATE INDEX idx_works_sort_title_id ON public.works USING btree (sort_title,
id);
...
'Enforces at most one in_progress writeback job per manifestation. Load-bearing for multi-replica correctness; see '
'services::writeback::queue::claim_next for the retry path on SQLSTATE 23505.';

CREATE INDEX idx_writeback_jobs_manifestation_status ON public.writeback_jobs USING btree (manifestation_id,
status);
...
ALTER TABLE ONLY public.reading_state
ADD CONSTRAINT reading_state_manifestation_id_fkey FOREIGN KEY (manifestation_id) REFERENCES public.manifestations(id)
ON DELETE CASCADE;
...
ALTER TABLE ONLY public.works
ADD CONSTRAINT works_language_version_id_fkey FOREIGN KEY (language_version_id) REFERENCES public.metadata_versions(id)
ON DELETE SET NULL;

ALTER TABLE ONLY public.works
ADD CONSTRAINT works_subtitle_version_id_fkey FOREIGN KEY (subtitle_version_id) REFERENCES public.metadata_versions(id)
ON DELETE SET NULL;
...
ALTER TABLE ONLY public.writeback_jobs
ADD CONSTRAINT writeback_jobs_manifestation_id_fkey FOREIGN KEY (manifestation_id) REFERENCES public.manifestations(id)
ON DELETE CASCADE;
...
CREATE POLICY manifestations_ingestion_full_access ON public.manifestations TO reverie_ingestion USING (true)
WITH CHECK (true);
...
WHERE ((si.manifestation_id = manifestations.id) AND (s.user_id = (( SELECT current_setting('app.current_user_id'::text,
...
CREATE POLICY work_external_identifiers_insert ON public.work_external_identifiers FOR INSERT TO reverie_app
...
INSERT,
DELETE,
UPDATE ON TABLE public.api_cache TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.ingestion_jobs TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.local_credentials TO reverie_app;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.local_login_throttle TO reverie_app;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_external_identifiers TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_external_identifiers TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_external_ratings TO reverie_ingestion;
GRANT SELECT ON TABLE public.manifestation_external_ratings TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_genres TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_genres TO reverie_ingestion;
GRANT SELECT ON TABLE public.manifestation_genres TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_moods TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_moods TO reverie_ingestion;
GRANT SELECT ON TABLE public.manifestation_moods TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_tags TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.manifestation_tags TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.manifestations TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.metadata_versions TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.metadata_versions TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.omnibus_contents TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.omnibus_contents TO reverie_ingestion;
GRANT SELECT ON TABLE public.omnibus_contents TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.password_reset_pins TO reverie_app;
...
INSERT,
DELETE,
UPDATE ON TABLE public.reading_sessions TO reverie_app;
...
INSERT,
DELETE,
UPDATE ON TABLE public.series_works TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE public.user_identities TO reverie_app;
...
INSERT,
DELETE,
UPDATE ON TABLE public.webhook_deliveries TO reverie_app;
GRANT SELECT ON TABLE public.webhook_deliveries TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.webhook_event_dedupe TO reverie_app;
...
INSERT,
DELETE,
UPDATE ON TABLE public.work_authors TO reverie_ingestion;
GRANT SELECT ON TABLE public.work_authors TO reverie_readonly;

GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.work_external_identifiers TO reverie_app;
GRANT SELECT,
INSERT,
DELETE,
UPDATE ON TABLE public.work_external_identifiers TO reverie_ingestion;
...
INSERT,
DELETE,
UPDATE ON TABLE tower_sessions.session TO reverie_app;
