...
## Reverie invocation and boundaries

Run commands from the repository root. Every `playwright-cli` command shown in this skill or its references means:
...
```bash
playwright-cli state-load /user/approved/path/auth-state.json
...
# launch the dashboard for UI review or design feedback; the user annotates the page and returns notes with the marked region
...
# search a large snapshot instead of capturing it all; this returns matching nodes
...
Restore the workspace dependency with the repository setup command:
...
playwright-cli fill e1 "Example name"
